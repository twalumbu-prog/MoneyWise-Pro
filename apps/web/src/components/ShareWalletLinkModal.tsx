...
const ShareWalletLinkModal: React.FC<ShareWalletLinkModalProps> = ({
    ...
    const handleDownloadQuickPay = async () => {
        ...
        const ORG = (organizationName || walletName || '').toUpperCase();
        ...
        const BADGE_SIZE = 16; const GAP = 6;
        const totalW = nameW + GAP + BADGE_SIZE;
        c.fillText(ORG, W / 2 - (GAP + BADGE_SIZE) / 2, NAME_Y);
        // Blue verified circle
        const bx = W / 2 + totalW / 2 - BADGE_SIZE + 2; const by = NAME_Y;
        c.fillStyle = '#2563eb';
        c.beginPath(); c.arc(bx, by, BADGE_SIZE / 2, 0, Math.PI * 2); c.fill();
        // White check mark
        c.strokeStyle = '#fff'; c.lineWidth = 1.5; c.lineCap = 'round'; c.lineJoin = 'round';
        c.beginPath();
        c.moveTo(bx - 3.5, by); c.lineTo(bx - 1, by + 2.5); c.lineTo(bx + 3.5, by - 2.5);
        c.stroke();
        ...
        const [airtelImg, mtnImg] = await Promise.all([loadSvg(AIRTEL_SVG_WHITE), loadSvg(MTN_SVG_WHITE)]);

        const LOGO_H = 18;
        // Airtel: viewBox ≈ 338×353 (slightly taller than wide → treat as square)
        const AIRTEL_W = Math.round(LOGO_H * (338 / 353));
        // MTN: viewBox 998×499 (2:1 wide)
        const MTN_W = Math.round(LOGO_H * (998 / 499));
        // MoneyWise: draw "MoneyWise" in Advercase
        c.font = `bold ${LOGO_H - 2}px Advercase, DM Sans, sans-serif`;
        const MW_W = c.measureText('MoneyWise').width;
        ...
        c.fillText('MoneyWise', lx, LOGOS_Y);
