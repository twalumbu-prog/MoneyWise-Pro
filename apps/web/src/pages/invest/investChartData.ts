import type { ProductType } from './InvestHome';
...
// ── Seeded RNG (deterministic per product so charts don't reshuffle on re-render) ──
...
const VOLATILITY: Record<ProductType, number> = {
    UNIT_TRUST: 0.016,
    BOND: 0.006,
    FIXED_DEPOSIT: 0.0012,
};

const HISTORICAL_GROWTH_RANGE: Record<ProductType, [number, number]> = {
 ...
 */
export function generateDailyHistory(productId: string, priceStr: string, ytdStr: string, type: ProductType): PricePoint[] {
    ...
    // Segment A: ~3 years ago -> Jan 1 this year
    ...
    // Segment B: Jan 1 this year -> today (reproduces the displayed YTD %)
