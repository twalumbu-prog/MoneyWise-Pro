/**
 * Authenticated API client, shared by the web app and the native app.
 *
 * Moved verbatim from apps/web/src/lib/api.ts. The resilience logic here was
 * hard-won against real auth-lock contention and flaky carriers, so it is
 * deliberately unchanged — only the four browser touchpoints were swapped for
 * the Platform adapter: env, supabase, telemetry, and now()/randomUUID().
 */
...
/**
 * Installed on first use rather than at module load. The web build used a
 * top-level side effect, which is safe under Vite's import order but would
 * race `configureCore` here — core modules must not touch the host before the
 * host has finished handing itself over.
 */
...
export async function apiFetch(path: string, options: RequestInit = {}): Promise<Response> {
    ...
    const baseProps = {
        workflow_id: core.randomUUID(),
        endpoint: normalizeEndpoint(path),
        method: (options.method || 'GET').toUpperCase(),
    };
    try {
        const response = await doApiFetch(path, options);
        core.telemetry.track('api', 'fetch', 'succeeded', {
            ...baseProps,
            status_code: response.status,
            duration_ms: Math.round(core.now() - start),
        });
        return response;
    } catch (err: any) {
        core.telemetry.track('api', 'fetch', 'failed', {
            ...baseProps,
            duration_ms: Math.round(core.now() - start),
...
async function doApiFetch(path: string, options: RequestInit = {}): Promise<Response> {
    ensureTokenCache();
    ...
    if (response.status === 401) {
        ...
        if (response.status === 401) {
            if (refresh.status === 'invalid') {
                ...
                await getCore().supabase.auth.signOut();
            } else {
...
/** Convenience for the overwhelmingly common `await (await apiFetch(x)).json()`. */
